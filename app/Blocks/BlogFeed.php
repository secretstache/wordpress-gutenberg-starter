...
class BlogFeed extends Block
{
    ...
    protected function prepareData($attributes, $content): array
    {
        ...
        $include_filters    = $attributes['isShowFilters'] ?? false;
        ...
        if ($include_filters && $query === self::QUERY_LATEST && class_exists('FacetWP')) {
        ...
        return [
            ...
            'posts'                 => $posts,
            'include_filters'       => $include_filters && $query === self::QUERY_LATEST && class_exists('FacetWP'),
        ];
    }
}
